...
>
class LinearProgramCPU : public BaseCPU<
  LinearProgramCPU<
    MEMORY_MODEL_T,
    // TAG_T,
    INST_ARGUMENT_T,
...
public:
  ...
  using program_t = sgp::cpu::linprg::LinearProgram<tag_t, arg_t>;
  // -- Memory model --
...
protected:
  ...
  // Configures default behavior for "Basic" flow type (e.g., if statements).
  void SetupDefaultFlowControl_Basic() {
    ...
    // ---- On break ----
    // - Pop current flow from the stack
...
public:
  ...
  LinearProgramCPU(LinearProgramCPU&&) = default;
  LinearProgramCPU(const LinearProgramCPU&) = default;
  ...
  size_t FindEndOfBlock(size_t mp, size_t ip) const {
    ...
    while (true) {
      ...
      seen.emplace(ip);
      ++ip;
      if (ip >= program.GetSize() && seen.size() < modules[mp].GetSize()) {
        ip %= program.GetSize();
      }
    }
    return ip;
  ...
  void CallModule(
    ...
    flow_handler.OpenFlow(
      *this,
      {flow_t::CALL, module_id, module_info.begin, module_info.begin, module_info.end},
      exec_state
    );
  ...
  void ReturnCall(exec_state_t & exec_state) {
    ...
    if (exec_state.call_stack.size() > 1) {
      ...
      memory_model.OnModuleReturn(
        returning_state.GetMemory(),
        caller_state.GetMemory()
      );
    }
  ...
  void UpdateModules() {
    ...
    for (size_t pos = 0; pos < program.GetSize(); ++pos) {
      ...
      if (inst_lib.HasProperty(inst.GetID(), InstProperty::MODULE)) {
        ...
        modules.emplace_back(
          mod_id,                                        // ID
          ( (pos+1) < program.GetSize() ) ? pos+1 : 0,   // Begin
          -1,                                            // End
          inst.GetTags()[0]                              // Tag
        );
      } else {
        ...
        if (modules.size()) {
          modules.back().in_module.emplace(pos);
        } else {
          dangling_instructions.emplace(pos);
        }
      }
    }
    ...
    for (size_t val : dangling_instructions) {
      modules.back().in_module.emplace(val);
    }
    // Reset matchbin
    ResetMatchBin();
  }
  ...
  /// module id.
  module_t& GetModule(size_t i) {
    emp_assert(i < modules.size());
    return modules[i];
  }
  ...
  void PrintModules(std::ostream& os=std::cout) const {
    ...
    for (size_t i = 0; i < modules.size(); ++i) {
      ...
      os << "{";
      os << "id:" << modules[i].id;
      os << ", begin:" << modules[i].begin;
      os << ", end:" << modules[i].end;
      os << ", tag:" << modules[i].tag;
      os << "}";
    }
    os << "]";
  }
