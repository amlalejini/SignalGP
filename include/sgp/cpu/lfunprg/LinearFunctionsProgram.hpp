...
/// A LinearFunction is a program module that associates a tag with a linear
/// sequence of instructions (represented as a LinearProgram)
template<
  typename TAG_T,
...
public:
  ...
  size_t GetSize() const {
    return inst_sequence.GetSize();
  }

  tag_t& GetTag(size_t id=0) {
    emp_assert(tags.size());
    return tags[id];
  }

  const tag_t& GetTag(size_t id=0) const {
    emp_assert(tags.size());
    return tags[id];
  }

  // WARNING: no safety features here!
  emp::vector<tag_t>& GetTags() {
    return tags;
  }

  const emp::vector<tag_t>& GetTags() const {
    return tags;
  }
...
/// LinearFunctionsProgram defines a program comprising a set of LinearFunction objects.
template<
  typename TAG_T,
  typename ARGUMENT_T=int
>
...
public:
  LinearFunctionsProgram(
    const emp::vector<function_t>& fseq=emp::vector<function_t>()
  ) :
    program(fseq)
  { ; }
  ...
  void PushFunction(
    const tag_t& tag,
    const linprg::LinearProgram<tag_t, arg_t>& seq=linprg::LinearProgram<tag_t, arg_t>()
  ) {
...
LinearFunction<emp::BitSet<TAG_WIDTH>, int> GenRandLinearFunction(
  ...
  return {
    utils::RandomBitSets<TAG_WIDTH>(rnd, num_func_tags),
    linprg::GenRandLinearProgram<HARDWARE_T, TAG_WIDTH>(
      rnd,
      inst_lib,
      inst_cnt_range,
...
LinearFunctionsProgram<emp::BitSet<TAG_WIDTH>, int> GenRandLinearFunctionsProgram(
  ...
  const size_t func_cnt = rnd.GetUInt(
    num_func_range.GetLower(), num_func_range.GetUpper()+1
  );
  LinearFunctionsProgram<emp::BitSet<TAG_WIDTH>, int> new_program;
  for (size_t fID = 0; fID < func_cnt; ++fID) {
    new_program.PushFunction(
      GenRandLinearFunction<HARDWARE_T, TAG_WIDTH>(
        rnd,
        inst_lib,
        num_func_tags,
        func_inst_cnt_range,
